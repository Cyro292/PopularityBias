"""Convert a Wikimedia pageview dump to an English Wikipedia parquet."""
...
def analyse_sources() -> dict[str, int]:
    """Count records by source wiki in the raw pageview dump."""
...
def main() -> None:
    """Create the pageview parquet or report statistics for an existing one."""
    ...
    if OUTPUT_PATH.exists():
        ...
        logger.info(f"Unique articles: {len(df)}")
        logger.info("Articles above one million views:\n%s", df[df["views"] > 1_000_000].head(20))
        return

    logger.info(f"Creating parquet file at: {OUTPUT_PATH}")
    create_parquet()

    logger.info("Deduplicating globally, one row per ID")
    df = pd.read_parquet(OUTPUT_PATH)
    df = df.groupby("id", as_index=False)["views"].sum()
    df.to_parquet(OUTPUT_PATH, index=False)
    logger.info(f"Done. {len(df):,} unique articles written")
