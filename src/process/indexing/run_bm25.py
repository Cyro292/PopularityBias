...
Builds a bm25s-backed index from a wiki corpus Parquet file and writes it to
disk.  The resulting directory can be loaded by
:class:`src.rag.bm25_rag_service.BM25RagService` for offline keyword retrieval.
...
::
    ...
    <output_dir>/
        ...
        arrays.npz    ← compact numpy metadata
        config.json   ← hyperparameters and corpus statistics

RAM requirements
----------------
Tokenising the full ``wiki_full_bil`` corpus (~30 M chunks) requires
**12–16 GB** peak RAM.  Query-time RAM after loading is ~500 MB.
...
::
    ...
    python -m src.process.indexing.run_bm25 --collection wiki_full_bil
    python -m src.process.indexing.run_bm25 \\
        --parquet data/wiki_full_bil/wiki_corpus.parquet \\
        --output-dir data/wiki_full_bil/bm25
    python -m src.process.indexing.run_bm25 --k1 1.2 --b 0.75
    python -m src.process.indexing.run_bm25 --no-chunk
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--output-dir", "-o", type=Path, default=None,
                   help="Index output directory (default: data/<collection>/bm25/).")
    p.add_argument("--no-chunk", action="store_true",
                   help="Index whole articles rather than splitting into chunks.")
    ...
    p.add_argument("--k1", type=float, default=DEFAULTS.k1,
                   help="BM25 term-saturation parameter (typical: 1.2–2.0).")
    p.add_argument("--b", type=float, default=DEFAULTS.b,
                   help="BM25 length-normalisation parameter (0 = off, 1 = full).")
    p.add_argument("--method", default=DEFAULTS.method,
                   choices=["lucene", "bm25+", "atire", "robertson", "bm25l"],
...
def run_indexing(
    ...
    *,
    ...
    Args:
        ...
        method: bm25s scoring method (e.g. ``"lucene"``, ``"bm25+"``).
        metadata_fields: Corpus columns stored as document metadata.

    Returns:
        Number of indexed chunks (or articles when ``chunk=False``).
    ...
    logger.info("  BM25       : k1=%.2f  b=%.2f  method=%s", k1, b, method)
    ...
    t0 = time.time()
    _index, n_indexed = service.index_from_parquet(
        parquet_path,
        text_field="text",
...
def main(argv: list[str] | None = None) -> None:
    """Parse arguments and run BM25 indexing."""
    logging.basicConfig(
        level=logging.INFO,
    ...
    output_dir: Path   = args.output_dir or (collection_dir / f"bm25_{args.method}")

    if output_dir.exists():
        logger.warning("Output directory %s already exists — files will be overwritten.", output_dir)
