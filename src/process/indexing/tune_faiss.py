...
python -m src.process.indexing.tune_faiss --m 48 --nlist 8192 --rebuild

# Dry run — print what would change without touching anything:
python -m src.process.indexing.tune_faiss --nprobe 256 --m 48 --dry-run

# Benchmark recall@10 with a quick sample after tuning:
python -m src.process.indexing.tune_faiss --nprobe 256 --benchmark
