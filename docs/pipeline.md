...
python -m src.process.indexing.tune_faiss --nprobe 256

# Rebuild with different PQ parameters (slow)
python -m src.process.indexing.tune_faiss --m 48 --nbits 12 --rebuild

# Benchmark recall@k
python -m src.process.indexing.tune_faiss --benchmark --k 10
```
...
python -m src.process.indexing.tune_faiss --m 48 --nbits 12 --nlist 8192 --rebuild

# Dry run (preview changes)
python -m src.process.indexing.tune_faiss --m 48 --rebuild --dry-run

# Benchmark recall
python -m src.process.indexing.tune_faiss --benchmark --k 10

# Benchmark with real QA questions
python -m src.process.indexing.tune_faiss --benchmark-qa --n-questions 500
```

### Export Elasticsearch to Parquet

```bash
python -m src.process.indexing.export_elasticsearch \
...
```bash
# Into parquet files
python -m src.process.migrations.backfill_decile_columns
...
python -m src.process.corpus.build_wiki_2026

# Clean MediaWiki markup
python -m src.process.corpus.clean_wiki_2026

# Convert raw pageviews to parquet
python -m src.process.corpus.convert_pageviews

# Find top new articles absent from old corpus
python -m src.process.corpus.report_new_2026_articles

# Manually select genuinely absent 2026 pages, retrieve comparable high-popularity
# controls from wiki_full_bil/faiss_high, and add authored pairs to
# data/similarity_pairs.json.

# Build comparable 2026 retrieval indices before scoring its target articles.
python -m src.process.indexing.run_faiss \
  --parquet data/wiki_2026/wiki_2026_corpus_clean.parquet \
  --output-dir data/wiki_2026/faiss_high \
  --metadata-fields id title views_2026
python -m src.process.indexing.run_bm25 \
  --parquet data/wiki_2026/wiki_2026_corpus_clean.parquet \
  --output-dir data/wiki_2026/bm25_bm25plus \
  --method bm25+ \
  --metadata-fields id title views_2026

# Score within each article's own corpus, then create paper figures.
...
3. Build analysis dataset
   └─ src.process.analysis.build_analysis_dataset → analysis_dataset.parquet
